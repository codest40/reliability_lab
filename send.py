import argparse
import sys

import requests


# ============================================================
# ARGUMENTS
# ============================================================

def parse_args():

    parser = argparse.ArgumentParser(
        description=(
            "Send a word to the SRE Lab receiver service."
        )
    )


    parser.add_argument(
        "message",
        help="Word or message to send",
    )


    parser.add_argument(
        "--action",
        type=str,
        default="normal",
        help=(
            "Behavior controller action "
            "(normal, slow, error, timeout)"
        ),
    )


    parser.add_argument(
        "--host",
        default="localhost",
        help="Receiver host",
    )


    parser.add_argument(
        "--port",
        type=int,
        default=5000,
        help="Receiver port",
    )


    parser.add_argument(
        "--timeoutClient",
        type=float,
        default=20,
        help="Client timeout in seconds",
    )


    parser.add_argument(
        "--timeoutService",
        type=float,
        default=5,
        help="Service B timeout in seconds",
    )


    return parser.parse_args()


# ============================================================
# MAIN
# ============================================================

def main():

    args = parse_args()


    url_post = (
        f"http://{args.host}:{args.port}/notes"
    )

    url_get = (
        f"http://{args.host}:{args.port}/all_notes"
    )


    payload = {
        "word":
            args.message,
        "action":
            args.action,
        "timeout":
            args.timeoutService,
    }


    try:

        if args.message == "all":

            url = url_get

            response = requests.get(
                url_get,
                timeout=args.timeoutClient,
            )

        else:

            url = url_post

            response = requests.post(
                url_post,
                json=payload,
                timeout=args.timeoutClient,
            )


    except requests.exceptions.Timeout:

        print(
            "ERROR: Request timed out."
        )

        sys.exit(1)


    except requests.exceptions.ConnectionError:

        print(
            "ERROR: Could not connect "
            f"to receiver at {url}"
        )

        sys.exit(1)


    except requests.exceptions.RequestException as error:

        print(
            f"ERROR: Request failed: {error}"
        )

        sys.exit(1)


    try:

        result = response.json()

    except ValueError as error:

        print(
            f"ERROR: {error}"
        )

        print(
            f"HTTP status: "
            f"{response.status_code}"
        )

        sys.exit(1)


    if response.ok:

        print("SUCCESS")

        print(
            f"Message: {args.message}"
        )

        print(
            f"Response: {result}"
        )

        return


    print("FAILED")

    print(
        f"HTTP status: "
        f"{response.status_code}"
    )

    print(
        f"Response: {result}"
    )

    sys.exit(1)


if __name__ == "__main__":

    main()
